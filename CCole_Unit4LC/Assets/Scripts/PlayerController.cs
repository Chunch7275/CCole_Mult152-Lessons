...
public class PlayerController : MonoBehaviour
{
    ...
    private void OnCollissionEnter(Collider collission)
    {
      if(HasPowerUp && collission.gameObject.CompareTag("Enemy"))
        {
            Debug.Log("Player Collission with Enemy");
            Rigidbody rbenemy = GetComponent<Rigidbody>();
            Vector3 AwayDir = collission.gameObject.transform.position - transform.position;
